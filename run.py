...
monetization_config_path = os.path.join(
    APP_ROOT,
    "generated",
    "monetization_config.json",
)
JOURNAL_MODERATOR_SESSION_KEY = "journal_moderator_active"
DEFAULT_JOURNAL_MODERATOR_PASSWORD_SHA256 = (
    "47764a404bec5ab5b8746232de0c7850482d43fa0492f8f0b7fb6f7a5c47eb85"
)
...
_PAGE_AD_CONFIGS = [
    ...
    },
    {
        "paths": {
            "/survival",
            "/survival/army-library",
            "/survival/army-library/offline",
            "/survival/army-library/binder",
            "/survival/meal-bible",
            "/culture",
        },
        "slot_key": "survival",
...
def _load_monetization_config():
    ...
    try:
        ...
        if isinstance(raw_config, dict):
            config["adsense_client_id"] = _normalize_adsense_client_id(
                raw_config.get("adsense_client_id"),
            )
    ...
    env_client_id = _normalize_adsense_client_id(
...
def inject_page_ad_config():
    ...
    page_ad_slot = (
        monetization_config["slots"].get(page_ad_config["slot_key"])
        or monetization_config["default_slot"]
    )
...
def _init_journal_db():
    ...
    for column_name, column_type, default_value in [
        ...
        with contextlib.suppress(Exception):
            cur.execute(
                f"ALTER TABLE journal ADD COLUMN {column_name} {column_type} NOT NULL DEFAULT {default_value}",
            )
    conn.commit()
    conn.close()
...
def _generate_recovery_key():
    raw_key = (
        base64.b32encode(secrets.token_bytes(10)).decode("ascii").rstrip("=").upper()
    )
    return "DCRK-" + "-".join(
        raw_key[index : index + 4] for index in range(0, AUTH_RECOVERY_KEY_LENGTH, 4)
    )
...
def _normalize_recovery_key(value):
    ...
    compact = compact.removeprefix("DCRK")
    if len(compact) != AUTH_RECOVERY_KEY_LENGTH:
        return None
    if any(char not in AUTH_RECOVERY_KEY_ALPHABET for char in compact):
        return None
    normalized = "DCRK-" + "-".join(
        compact[index : index + 4] for index in range(0, AUTH_RECOVERY_KEY_LENGTH, 4)
    )
...
def _sanitize_auth_state_payload(storage):
    ...
    for key, value in storage.items():
        ...
        if (
            len(sanitized) > AUTH_MAX_STATE_KEYS
            or total_chars > AUTH_MAX_STATE_TOTAL_CHARS
        ):
...
CSP_POLICY = "default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdnjs.cloudflare.com https://unpkg.com https://pagead2.googlesyndication.com https://partner.googleadservices.com https://googleads.g.doubleclick.net https://*.adtrafficquality.google; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://unpkg.com https://cdnjs.cloudflare.com; font-src 'self' https://fonts.gstatic.com data:; img-src 'self' data: blob: https: http:; media-src 'self' blob: data: https: http:; connect-src 'self' https: http:; frame-src 'self' https: http:; worker-src 'self' blob:; manifest-src 'self'; upgrade-insecure-requests; block-all-mixed-content"
...
def _sanitize_project_name(value):
    safe_value = input_validator.sanitize_filename(
        value,
        max_length=MAX_PROJECT_NAME_CHARS,
    )
    if not safe_value:
        return None
    return safe_value
...
def _is_public_scan_target(hostname):
    ...
    if lowered in DISALLOWED_PRIVATE_HOSTS or lowered.endswith(
        (".local", ".internal", ".lan", ".home"),
    ):
        return False
    try:
        addresses = {
            info[4][0] for info in socket.getaddrinfo(hostname, None) if info[4]
        }
...
def _normalize_public_scan_url(raw_value):
    ...
    if parsed.scheme not in {"http", "https"} or not _is_public_scan_target(
        parsed.hostname,
    ):
        return None
    return validated
...
def _apply_runtime_response_policies(response):
    ...
    if (
        ...
        or request.path in SENSITIVE_NO_STORE_PATHS
        or (getattr(g, "current_user", None) and response.mimetype == "text/html")
    ):
        response.headers["Cache-Control"] = "no-store"
        response.headers["Pragma"] = "no-cache"
    elif request.path.startswith("/static/") and request.path.endswith(
        LONG_CACHE_STATIC_EXTENSIONS,
    ):
...
TOPLINE_SOURCE_SPECS = (
    ...
    },
    {
        ...
        "urls": ("https://endtimeheadlines.org/feed/",),
    },
)
HOURLY_SCRIPTURES = (
    {
    ...
    {
        "ref": "Matthew 24:6",
        "text": "Ye shall hear of wars and rumours of wars: see that ye be not troubled.",
    },
    {
        "ref": "Luke 21:28",
        "text": "When these things begin to come to pass, then look up, and lift up your heads.",
    },
    {
        "ref": "John 14:27",
...
def _fetch_topline_source(source, limit=4):
    for url in source["urls"]:
        ...
        if not payload:
            continue
        if (
            "Just a moment..." in payload
            or "Enable JavaScript and cookies to continue" in payload
        ):
            continue
...
DAILY_BIBLE_VERSES = (
    ...
    {
        "ref": "2 Timothy 3:16",
        "text": "All scripture is given by inspiration of God, and is profitable for doctrine, for reproof, for correction.",
    },
    {
        "ref": "Jeremiah 29:11",
    ...
    {
        "ref": "Matthew 24:14",
        "text": "This gospel of the kingdom shall be preached in all the world for a witness unto all nations.",
    },
    {
        "ref": "Daniel 1:17",
        "text": "God gave them knowledge and skill in all learning and wisdom.",
    },
    {
        "ref": "1 John 4:4",
...
def api_journal():
    ...
    conn.close()
    return jsonify(
        {"ok": True, "entries": rows, "moderation_enabled": moderation_enabled},
    )
...
def api_journal_add():
    ...
    if not user:
        return jsonify(
            {"ok": False, "error": "sign in to post to the public journal"},
        ), 401
    ...
    author = (
        input_validator.validate_string(user.get("username"), max_length=80)
        or "Anonymous"
    )
...
def api_advertise_inquiry():
    ...
    contact_name = input_validator.validate_string(
    ...
    start_window = (
        input_validator.validate_string(data.get("start_window"), max_length=120) or ""
    )
...
def api_auth_register():
    ...
    existing = conn.execute(
        "SELECT id FROM users WHERE username_lookup = ?",
        (lookup,),
    ).fetchone()
    if existing:
        conn.close()
    ...
    cursor.execute(
        ...
        (
            username,
            lookup,
            generate_password_hash(password, method=AUTH_PASSWORD_HASH_METHOD),
            now,
            now,
            now,
        ),
...
def api_auth_login():
    ...
    if not username or not password:
        return jsonify(
            {"ok": False, "error": "login id and passcode are required"},
        ), 400
...
def api_auth_reset_password():
    ...
    if not username or not recovery_key or not password:
        return jsonify(
            {
                "ok": False,
                "error": "login id, recovery key, and new passcode are required",
            },
        ), 400
    ...
    if (
        ...
        or not check_password_hash(row["recovery_hash"], recovery_key)
    ):
        conn.close()
...
def _offline_bundle_readme(has_vendor):
    vendor_note = (
        ...
        if has_vendor
        else "Vendored Python packages were not present when this bundle was created.\n"
...
def service_worker():
    response = send_file(
        os.path.join(APP_ROOT, "static", "sw.js"),
...
def site_manifest():
    response = send_file(
        os.path.join(APP_ROOT, "static", "site.webmanifest"),
...
def robots_txt():
    response = send_file(
        os.path.join(APP_ROOT, "static", "robots.txt"),
        mimetype="text/plain",
    )
...
def ads_txt():
    response = send_file(
        os.path.join(APP_ROOT, "static", "ads.txt"),
        mimetype="text/plain",
    )
    response.headers["Cache-Control"] = "public, max-age=86400"
    return response
...
def sitemap_xml():
    response = send_file(
        os.path.join(APP_ROOT, "static", "sitemap.xml"),
...
def bing_site_auth():
    response = send_file(
        os.path.join(APP_ROOT, "static", "BingSiteAuth.xml"),
        mimetype="application/xml",
    )
    response.headers["Cache-Control"] = "public, max-age=86400"
    return response
...
def run_code():
    ...
    language = (
        input_validator.validate_string(
            data.get("language"),
            max_length=16,
            allow_unicode=False,
        )
        ...
        if language == "python":
            ...
            try:
                run_proc = subprocess.run(
                    [java, "-Djava.awt.headless=true", "-cp", ".", "Main"],
                    **_subprocess_kwargs(tempdir, 5),
                )
                return jsonify(
                    {
            ...
            if css is None or js is None:
                return jsonify(
                    {
                        "error": f"Preview payload too large (max {MAX_CODE_CHARS} chars per section)",
                    },
                ), 400
            ...
            if html_code is None:
                return jsonify(
                    {"error": f"HTML preview too large (max {MAX_CODE_CHARS} chars)"},
                ), 400
            if not html_code:
...
def agent_route():
    ...
    instruction = input_validator.validate_string(
        data.get("instruction"),
        max_length=MAX_AGENT_INSTRUCTION_CHARS,
    )
    project_name = (
        _sanitize_project_name(data.get("project_name"))
    ...
    if execute and not confirm:
        return jsonify(
            {
                "error": "Execution requested but not confirmed. Set confirm=true to execute.",
            },
        ), 400
...
def download_multiple():
    ...
    return send_file(
        ...
        mimetype="application/zip",
        as_attachment=True,
        download_name="delta_projects.zip",
    )
...
def download_offline_site():
    ...
    optional_items = [
        name
        for name in OPTIONAL_OFFLINE_BUNDLE_ITEMS
        ...
        for relative_path in (*bundle_items, *optional_items):
            ...
            if os.path.exists(abs_path):
                _write_bundle_path(
                    zf,
                    abs_path,
                    os.path.join(OFFLINE_BUNDLE_ROOT, relative_path),
                )

        zf.writestr(
            f"{OFFLINE_BUNDLE_ROOT}/README_OFFLINE.txt",
            _offline_bundle_readme(has_vendor),
        )
        zf.writestr(
            f"{OFFLINE_BUNDLE_ROOT}/start_delta.command",
            _offline_bundle_launcher_sh(),
        )
        zf.writestr(
            f"{OFFLINE_BUNDLE_ROOT}/start_delta.bat",
            _offline_bundle_launcher_bat(),
        )
        zf.writestr(
            f"{OFFLINE_BUNDLE_ROOT}/install_delta.command",
            _offline_bundle_installer_sh(),
        )
        zf.writestr(
            f"{OFFLINE_BUNDLE_ROOT}/install_delta.bat",
            _offline_bundle_installer_bat(),
        )
    ...
    return send_file(
        ...
        mimetype="application/zip",
        as_attachment=True,
        download_name="delta_coding_offline_app.zip",
    )
...
FREE_AI_MODELS = [
    {
        "id": "openai",
        "name": "GPT-OSS 20B",
        "desc": "Key-free reasoning model served through Delta backend",
        "provider": "pollinations",
    },
    {
        "id": "openai-fast",
        "name": "GPT-OSS 20B Fast",
        "desc": "Key-free faster mode for quicker responses",
        "provider": "pollinations",
    },
]
OPTIONAL_HF_MODELS = [
    {
        ...
        "name": "Mistral 7B (HF)",
        "desc": "Available when HF_API_TOKEN is configured",
...
def api_ai():
    ...
    data = _json_payload()
    prompt = input_validator.validate_string(
        data.get("prompt"),
    ...
    max_tokens = input_validator.validate_integer(
        data.get("max_tokens", 512),
        min_val=1,
        max_val=2048,
    )
    temperature = input_validator.validate_float(
        data.get("temperature", 0.7),
        min_val=0.0,
        max_val=2.0,
    )
    if (
        provider not in ALLOWED_AI_PROVIDERS
    ...
    messages = [
        {
            "role": "system",
            "content": "You are Delta AI, a tactical coding assistant. Provide clear, precise answers with code examples when relevant.",
        },
...
def _call_pollinations(model, messages, max_tokens, temperature):
    ...
    try:
        ...
        return jsonify(
            {"ok": True, "text": text, "model": used_model, "provider": "pollinations"},
        )
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 502
...
def _call_hf(model, messages, max_tokens, temperature):
    ...
    if not hf_token:
        return jsonify(
            {
                "ok": False,
                "error": "HF_API_TOKEN not set. Get a free token at huggingface.co/settings/tokens",
            },
        ), 400
    try:
        resp = http_requests.post(
        ...
        if resp.status_code == 503:
            ...
            return jsonify(
                {
                    "ok": False,
                    "error": f"Model loading, retry in ~{int(wait)}s",
                    "loading": True,
                },
            ), 503
        resp.raise_for_status()
        ...
        text = result.get("choices", [{}])[0].get("message", {}).get("content", "")
        return jsonify(
            {"ok": True, "text": text, "model": model, "provider": "huggingface"},
        )
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 502
...
def api_truth_summary():
    ...
    data = _json_payload()
    query = input_validator.validate_string(
        data.get("query"),
        max_length=MAX_SUMMARY_QUERY_CHARS,
    )
    if not query:
...
def share_code():
    ...
    if (
        language not in {"python", "java", "html", "css", "web"}
        or code is None
        or css is None
        or js is None
    ):
...
def check_headers():
    ...
    if not url:
        return jsonify(
            {"ok": False, "error": "public http(s) url parameter required"},
        ), 400
...
def cyber_resources():
    ...
    resources = [
        ...
        },
        {
            ...
            "desc": "Check if your email has been in a data breach",
        },
        {
            "name": "SSL Labs",
            "url": "https://www.ssllabs.com/ssltest/",
            "category": "SSL Scanner",
            "desc": "Deep analysis of SSL/TLS configuration",
        },
        {
            "name": "SecurityHeaders.com",
            "url": "https://securityheaders.com/",
            "category": "Headers",
            "desc": "Analyze HTTP response headers",
        },
        {
            "name": "VirusTotal",
            "url": "https://www.virustotal.com/",
            "category": "Malware",
            "desc": "Scan files and URLs for malware",
        },
        {
            "name": "Shodan",
        ...
        },
        {
            ...
            "category": "Encryption",
            "desc": "Web app for encryption, encoding, compression",
        },
        {
            "name": "Exploit Database",
            "url": "https://www.exploit-db.com/",
            "category": "Exploits",
            "desc": "Archive of public exploits and software",
        },
        {
            "name": "NIST NVD",
        ...
        },
        {
            ...
            "category": "Practice",
            "desc": "Capture The Flag competitions and writeups",
        },
        {
            "name": "TryHackMe",
            "url": "https://tryhackme.com/",
            "category": "Practice",
            "desc": "Free hands-on cybersecurity training",
        },
        {
            "name": "HackTheBox",
            "url": "https://www.hackthebox.com/",
            "category": "Practice",
            "desc": "Penetration testing labs and challenges",
        },
        {
            "name": "CRT.sh",
            "url": "https://crt.sh/",
        ...
        },
        {
            ...
            "category": "Recon",
            "desc": "Free domain research and DNS recon",
        },
        {
            "name": "Let's Encrypt",
            "url": "https://letsencrypt.org/",
            "category": "SSL",
            "desc": "Free SSL/TLS certificates",
        },
        {
            "name": "Mozilla Observatory",
            "url": "https://observatory.mozilla.org/",
            "category": "Scanner",
            "desc": "Free website security scanner",
        },
    ]
...
def map_live_satellite_availability():
    ...
    if frames is None or tiles is None:
        return jsonify(
            {"ok": False, "error": "invalid availability probe payload"},
        ), 400
    ...
    available_frames = [
        ...
        if all(
            _live_satellite_tile_available(source_key, frame_time, tile)
            for tile in tiles
        )
    ]
...
ARMY_SURVIVAL_LIBRARY_PDF_DIR = os.path.join(
    APP_ROOT,
    "static",
    "manuals",
    "army-library",
...
def _get_army_survival_manuals_for_categories(category_slugs):
    ...
    return [
        manual
        for manual in ARMY_SURVIVAL_MANUALS
        if manual["category"] in normalized_categories
    ]


def _army_survival_manual_filename(manual):
    filename_root = secure_filename(
        manual.get("title") or manual.get("slug") or "army-survival-manual",
    )
...
def _army_survival_requested_manual_slugs():
    ...
    seen = set()
    for raw_value in [
        *request.args.getlist("manual"),
        *((request.args.get("manuals") or "").split(",")),
    ]:
...
def _selected_army_survival_manuals_from_request(default_to_all=False):
    ...
    for manual in ARMY_SURVIVAL_MANUALS:
        if (
            requested_scope == "all"
            or manual["category"] in requested_categories
            or manual["slug"] in requested_manuals
        ):
            selected.append(manual)
    return selected


def _army_survival_selection_url(
    endpoint,
    scope="",
    categories=None,
    manual_slugs=None,
...
def _group_army_survival_manuals_by_category(manuals):
    ...
    for category in ARMY_SURVIVAL_MANUAL_CATEGORIES:
        category_manuals = [
            manual for manual in manuals if manual["category"] == category["slug"]
        ]
        if category_manuals:
            grouped.append(
                {
...
def survival_army_library_page():
    ...
    active_manual = (
        _get_army_survival_manual(requested_slug) or ARMY_SURVIVAL_MANUALS[0]
    )
    return render_template(
        ...
        active_manual_binder_url=_army_survival_binder_url(
            manual_slugs=[active_manual["slug"]],
        ),
...
def survival_army_library_offline_page():
    category_lookup = {
        entry["slug"]: entry["label"] for entry in ARMY_SURVIVAL_MANUAL_CATEGORIES
    }
    bundles = []
        ...
        bundles.append(
            {
                **bundle,
                ...
                "category_labels": [
                    ...
                    for slug in bundle.get("categories", [])
...
def survival_army_library_binder():
    ...
    category_labels = [
        category_lookup[slug]
        for slug in requested_categories
        if slug in category_lookup
    ]
    full_shelf_requested = requested_scope == "all" or (
        not requested_categories and not requested_manual_slugs
    )
    ...
    return render_template(
        ...
        full_shelf_download_url=_army_survival_download_url(scope="all"),
        full_shelf_binder_url=_army_survival_binder_url(scope="all"),
        primary_reader_url=url_for(
            "survival_army_library_page",
            manual=selected_manuals[0]["slug"],
        )
        if selected_manuals
        else url_for("survival_army_library_page"),
    )
...
def survival_army_library_pdf(manual_slug):
    ...
    attachment_requested = (request.args.get("download") or "").strip().lower() in {
        "1",
        "true",
        "yes",
    }
    ...
    response.headers["Content-Disposition"] = (
        f'{disposition}; filename="{_army_survival_manual_filename(manual)}"'
    )
...
def survival_army_library_download():
    ...
    with zipfile.ZipFile(
        ...
        for manual in selected_manuals:
            ...
            try:
                ...
                with archive.open(
                    _army_survival_manual_filename(manual),
                    mode="w",
                ) as zipped_entry:
                    for chunk in upstream.iter_content(chunk_size=65536):
                        if chunk:
        ...
        if failed:
            ...
            readme_lines.extend(
                f"- {entry['title']}: {entry['error']}" for entry in failed
            )
    ...
    if not downloaded:
        archive_file.close()
        return jsonify(
            {"ok": False, "error": "all selected manuals failed to download"},
        ), 502
...
def _load_mechanics_catalog():
    try:
        ...
        if not items:
            msg = "manual store returned no mechanics entries"
        ...
        for brand in brands:
            for model in models_by_brand.get(brand, []):
                years_by_brand_model[(brand, model)] = manual_store.list_years(
                    brand,
                    model,
                )
        ...
        for brand in brands:
            ...
            for model in models:
                years_by_brand_model[(brand, model)] = sorted(
                    {item["year"] for item in brand_items if item["model"] == model},
                )
        return items, brands, models_by_brand, years_by_brand_model
...
def mechanics_page():
    ...
    featured_brands.sort(
        key=lambda entry: (-entry["years"], -entry["models"], entry["brand"]),
    )
...
def _build_mechanics_reference_rows(items):
    ...
    for item in items:
        ...
        indexed_entry["has_pdf"] = indexed_entry["has_pdf"] or bool(
            item.get("pdf_path"),
        )
    ...
    reference_rows = [
        ...
        },
        {
            "decade": "2000s",
            "year": "2008",
            "brand": "Force Protection",
            "model": "Cougar 4x4 MRAP",
        ...
        # ── NAVAL ────────────────────────────────────────────────────────────
        {
            "decade": "2000s",
            "year": "2005",
            "brand": "Huntington Ingalls",
            "model": "DDG-51 Arleigh Burke",
        ...
        reference_rows.append(
            {
                ...
                "brand": brand,
                "model": model,
                "segment": item.get("title")
                or f"{brand} {model} indexed workbench entry",
                "layout": "Indexed manual track",
                "powertrain": description
                or "Powertrain details are stored in the linked indexed documents for this vehicle.",
                ...
                or "This row is generated from the current manual store and links directly into the mechanics workbench.",
            },
        )
    ...
    for row in reference_rows:
        ...
        row["status"] = (
            ...
            + (
                " / PDF attached"
                if indexed_entry["has_pdf"]
                else " / detail page ready"
            )
            if indexed_entry
            else "Reference profile"
        )
...
_BLUEPRINT_GALLERY = [
    # AVIATION
    {
        ...
        "img_type": "Blueprint 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Boeing_AH-64_Apache",
        "official": "https://www.boeing.com/defense/apache-helicopter/",
        "license": "PD-USGov",
        "tm": "TM 1-1520-238-10",
        "has_blueprint": True,
    },
    {
        ...
        "img_type": "Orthographic 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Sikorsky_UH-60_Black_Hawk",
        "official": "https://www.lockheedmartin.com/en-us/products/uh-60-black-hawk.html",
    ...
    },
    {
        ...
        "official": "https://www.boeing.com/defense/chinook/",
        "license": "CC-BY 3.0",
        "tm": "TM 1-1520-240-10",
        "has_blueprint": True,
    },
    {
        ...
        "img_type": "Blueprint 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Bell_AH-1Z_Viper",
        "official": "https://www.bellflight.com/products/bell-ah-1z",
        "license": "CC-BY 3.0",
        "tm": "NAVAIR 01-H57BJ-1",
        "has_blueprint": True,
    },
    {
        "id": "f35a",
        ...
        "official": "https://www.lockheedmartin.com/en-us/products/f-35.html",
    ...
    {
        "id": "f35b",
        "platform": "F-35B Lightning II",
        "maker": "Lockheed Martin",
        "category": "air",
        "type": "Multirole Fighter (STOVL)",
        "img": "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cc/Lockheed_Martin_F-35A_Lightning_II_3-view_drawing.png/1115px-Lockheed_Martin_F-35A_Lightning_II_3-view_drawing.png",
        "img_type": "Blueprint 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Lockheed_Martin_F-35_Lightning_II",
        "official": "https://www.lockheedmartin.com/en-us/products/f-35.html",
    ...
    {
        "id": "f35c",
        "platform": "F-35C Lightning II",
        "maker": "Lockheed Martin",
        "category": "air",
        "type": "Multirole Fighter (CV)",
        "img": "https://upload.wikimedia.org/wikipedia/commons/thumb/c/cc/Lockheed_Martin_F-35A_Lightning_II_3-view_drawing.png/1115px-Lockheed_Martin_F-35A_Lightning_II_3-view_drawing.png",
        "img_type": "Blueprint 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Lockheed_Martin_F-35_Lightning_II",
        "official": "https://www.lockheedmartin.com/en-us/products/f-35.html",
    ...
    },
    {
        ...
        "official": "https://www.lockheedmartin.com/en-us/products/f-22.html",
        "license": "PD-USGov",
        "tm": "T.O. 1F-22A-1",
        "has_blueprint": True,
    },
    {
        ...
        "img_type": "Blueprint 3-View",
        "wiki": "https://en.wikipedia.org/wiki/Boeing_F-15EX_Eagle_II",
        "official": "https://www.boeing.com/defense/f-15ex-eagle-ii/",
        "license": "PD-USGov",
        "tm": "T.O. 1F-15E-1",
        "has_blueprint": True,
    },
    {
        ...
        "img_type": "Dimensioned Sketch",
        "wiki": "https://en.wikipedia.org/wiki/General_Atomics_MQ-9_Reaper",
        "official": "https://www.ga-asi.com/remotely-piloted-aircraft/mq-9a",
        "license": "Public Domain",
        "tm": "T.O. 1MQ-9A-1",
        "has_blueprint": True,
    },
    ...
    {
        "id": "ddg51",
        "platform": "DDG-51 Arleigh Burke",
        "maker": "Huntington Ingalls",
        "category": "naval",
        ...
        "img_type": "Profile Diagram",
        "wiki": "https://en.wikipedia.org/wiki/Arleigh_Burke-class_destroyer",
        "official": "https://www.navy.mil/Resources/Fact-Files/Display-FactFiles/Article/2169512/ddg-51-arleigh-burke-class/",
        "license": "PD-USGov",
        "tm": "NAVSEA S9AA0-AA-SPN-010",
        "has_blueprint": True,
    },
    {
        "id": "cvn78",
        "platform": "CVN-78 Gerald R. Ford",
        "maker": "Huntington Ingalls",
        "category": "naval",
        ...
        "img_type": "Side Profile",
        "wiki": "https://en.wikipedia.org/wiki/Gerald_R._Ford-class_aircraft_carrier",
        "official": "https://www.navy.mil/Resources/Fact-Files/Display-FactFiles/Article/2169581/cvn-aircraft-carriers/",
        "license": "CC-BY-SA 4.0",
        "tm": "NAVSEA CVN-78",
        "has_blueprint": True,
    },
    {
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Freedom-class_littoral_combat_ship",
        "official": "https://www.navy.mil/Resources/Fact-Files/Display-FactFiles/Article/2169590/lcs-littoral-combat-ship/",
        "license": "PD-USGov",
        "tm": "NAVSEA LCS-1",
        "has_blueprint": False,
    },
    {
        "id": "lcs2",
        "platform": "LCS-2 Independence-class",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Independence-class_littoral_combat_ship",
        "official": "https://www.navy.mil/Resources/Fact-Files/Display-FactFiles/Article/2169590/lcs-littoral-combat-ship/",
        "license": "PD-USGov",
        "tm": "NAVSEA LCS-2",
        "has_blueprint": False,
    },
    {
        ...
        "img_type": "Official Illustration",
        "wiki": "https://en.wikipedia.org/wiki/Virginia-class_submarine",
        "official": "https://www.navy.mil/Resources/Fact-Files/Display-FactFiles/Article/2169558/ssn-virginia-class/",
        "license": "PD-USGov",
        "tm": "NAVSEA SSN-774",
        "has_blueprint": False,
    },
    ...
    {
        "id": "abrams",
        "platform": "M1A2 SEP Abrams",
        "maker": "General Dynamics",
        "category": "ground",
        "type": "Main Battle Tank",
        "img": "https://upload.wikimedia.org/wikipedia/commons/thumb/0/0b/M1A2_SEP_v3.jpg/960px-M1A2_SEP_v3.jpg",
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/M1_Abrams",
        "official": "https://www.gd.com/products/land-systems/m1-abrams",
        "license": "PD-USGov",
        "tm": "TM 9-2350-255-10",
        "has_blueprint": False,
    },
    {
        "id": "stryker",
        "platform": "Stryker ICV (M1126)",
        "maker": "General Dynamics",
        "category": "ground",
        "type": "Infantry Carrier Vehicle",
        "img": "https://upload.wikimedia.org/wikipedia/commons/thumb/5/51/Stryker_ICV_front_q.jpg/800px-Stryker_ICV_front_q.jpg",
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Stryker",
        "official": "https://www.gd.com/products/land-systems/stryker",
        "license": "PD-USGov",
        "tm": "TM 9-2350-405-10",
        "has_blueprint": False,
    },
    {
        ...
        "official": "https://www.bae-systems.com/en-us/products/fighting-vehicles/bradley",
        "license": "PD-USGov",
        "tm": "TM 9-2350-294-10",
        "has_blueprint": False,
    },
    {
        "id": "ampv",
        "platform": "AMPV (M1283/M1284)",
        "maker": "BAE Systems",
        "category": "ground",
        "type": "Armored Multi-Purpose Vehicle",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Armored_Multi-Purpose_Vehicle",
        "official": "https://www.bae-systems.com/en-us/products/armored-multi-purpose-vehicle-ampv",
        "license": "PD-USGov",
        "tm": "TM 9-2350-408-10",
        "has_blueprint": False,
    },
    {
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Humvee",
        "official": "https://www.amgeneral.com/vehicles/humvee/",
        "license": "PD-USGov",
        "tm": "TM 9-2320-280-10",
        "has_blueprint": False,
    },
    {
        "id": "matv",
        "platform": "Oshkosh M-ATV",
        "maker": "Oshkosh",
        "category": "ground",
        "type": "Mine-Resistant All-Terrain Vehicle",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Oshkosh_M-ATV",
        "official": "https://oshkoshdefense.com/vehicles/joint-light-tactical-vehicles/m-atv/",
        "license": "PD-USGov",
        "tm": "TM 9-2355-xxx-10",
        "has_blueprint": False,
    },
    {
        "id": "jltv",
        "platform": "Oshkosh JLTV",
        "maker": "Oshkosh",
        "category": "ground",
        "type": "Joint Light Tactical Vehicle",
        "img": "https://upload.wikimedia.org/wikipedia/commons/thumb/6/63/L-ATV_4.jpg/960px-L-ATV_4.jpg",
        "img_type": "Manufacturer Photo",
        "wiki": "https://en.wikipedia.org/wiki/Joint_Light_Tactical_Vehicle",
        "official": "https://oshkoshdefense.com/vehicles/joint-light-tactical-vehicles/jltv/",
        "license": "CC-BY-SA 4.0",
        "tm": "TM 9-2355-yyy-10",
        "has_blueprint": False,
    },
    {
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Family_of_Medium_Tactical_Vehicles",
        "official": "https://oshkoshdefense.com/vehicles/family-of-medium-tactical-vehicles/fmtv/",
        "license": "PD-USGov",
        "tm": "TM 9-2320-335-10",
        "has_blueprint": False,
    },
    {
        "id": "hemtt",
        "platform": "Oshkosh HEMTT",
        "maker": "Oshkosh",
        "category": "ground",
        "type": "Heavy Expanded Mobility Tactical Truck",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Heavy_Expanded_Mobility_Tactical_Truck",
        "official": "https://oshkoshdefense.com/vehicles/tactical-trucks/hemtt/",
        "license": "CC-BY-SA 4.0",
        "tm": "TM 9-2320-279-10",
        "has_blueprint": False,
    },
    {
        "id": "cougar",
        "platform": "Cougar MRAP",
        "maker": "Force Protection",
        "category": "ground",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Cougar_(MRAP)",
        "official": "https://www.textron.com/defense/systems/tactical-and-combat-vehicles",
        "license": "PD-USGov",
        "tm": "TM 9-2355-aaa-10",
        "has_blueprint": False,
    },
    {
        "id": "maxxpro",
        "platform": "Navistar MaxxPro MRAP",
        "maker": "Navistar",
        "category": "ground",
        "type": "Mine-Resistant Ambush Protected",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/Navistar_MaxxPro",
        "official": "https://www.internationaltruck.com/defense/",
        "license": "PD-USGov",
        "tm": "TM 9-2355-bbb-10",
        "has_blueprint": False,
    },
    {
        "id": "mrzr",
        "platform": "Polaris MRZR-D4",
        "maker": "Polaris",
        "category": "ground",
        "type": "Ultralight Tactical Vehicle",
        ...
        "img_type": "Air Show Photo",
        "wiki": "https://en.wikipedia.org/wiki/MRZR",
        "official": "https://www.polaris.com/en-us/off-road/defense/",
        "license": "CC-BY-SA 4.0",
        "tm": "Polaris MRZR-D4 Service Manual",
        "has_blueprint": False,
    },
    {
        "id": "m1117",
        "platform": "M1117 Guardian ASV",
        "maker": "Textron",
        "category": "ground",
        ...
        "img_type": "Official Photo",
        "wiki": "https://en.wikipedia.org/wiki/M1117_armored_security_vehicle",
        "official": "https://www.textron.com/defense/systems/tactical-and-combat-vehicles",
        "license": "PD-USGov",
        "tm": "TM 9-2355-ccc-10",
        "has_blueprint": False,
...
def mechanics_gallery():
    ...
    return render_template(
        "mechanics_gallery.html",
        blueprints=_BLUEPRINT_GALLERY,
...
def agents_page():
    import json as _json
    divisions = [
        {"key":"academic","label":"Academic","color":"#8B5CF6","icon_char":"🎓"},
        {"key":"design","label":"Design","color":"#EC4899","icon_char":"🎨"},
        {"key":"engineering","label":"Engineering","color":"#3B82F6","icon_char":"💻"},
        {"key":"finance","label":"Finance","color":"#22C55E","icon_char":"💰"},
        {"key":"game-development","label":"Game Dev","color":"#A855F7","icon_char":"🎮"},
        {"key":"gis","label":"GIS","color":"#14B8A6","icon_char":"🗺️"},
        {"key":"marketing","label":"Marketing","color":"#F97316","icon_char":"📣"},
        {"key":"paid-media","label":"Paid Media","color":"#EAB308","icon_char":"🎯"},
        {"key":"product","label":"Product","color":"#D946EF","icon_char":"📦"},
        {"key":"project-management","label":"Proj Mgmt","color":"#0EA5E9","icon_char":"📋"},
        {"key":"sales","label":"Sales","color":"#10B981","icon_char":"📈"},
        {"key":"security","label":"Security","color":"#EF4444","icon_char":"🛡️"},
        {"key":"spatial-computing","label":"Spatial","color":"#06B6D4","icon_char":"🥽"},
        {"key":"specialized","label":"Specialized","color":"#6366F1","icon_char":"✨"},
        {"key":"support","label":"Support","color":"#84CC16","icon_char":"🛟"},
        {"key":"testing","label":"Testing","color":"#F59E0B","icon_char":"🧪"},
    ]
    tools = [
        {"label":"Claude Code","color":"#D97757"},
        {"label":"Codex","color":"#10A37F"},
        {"label":"Gemini CLI","color":"#4285F4"},
        {"label":"GitHub Copilot","color":"#6E40C9"},
        {"label":"Qwen Code","color":"#615CED"},
        {"label":"Cursor","color":"#1F2430"},
        {"label":"opencode","color":"#FF6B35"},
        {"label":"Aider","color":"#8B5CF6"},
        {"label":"Antigravity","color":"#0EA5E9"},
        {"label":"Windsurf","color":"#09B6A2"},
        {"label":"Kimi","color":"#0F0F12"},
        {"label":"Osaurus","color":"#10B981"},
        {"label":"OpenClaw","color":"#E11D48"},
        {"label":"Hermes","color":"#7C3AED"},
    ]
    agents = [
        # ── ACADEMIC ──────────────────────────────────────────────
        {"id":"ac1","div":"academic","emoji":"🧠","name":"Psychologist","vibe":"People don't do things for no reason — I find the reason","desc":"Expert in human behavior, personality theory, motivation, and cognitive patterns — builds psychologically credible characters and interactions grounded in clinical and research frameworks"},
        {"id":"ac2","div":"academic","emoji":"🌍","name":"Anthropologist","vibe":"No culture is random — every practice is a solution to a problem you might not see yet","desc":"Expert in cultural systems, rituals, kinship, belief systems, and ethnographic method — builds culturally coherent societies that feel lived-in rather than invented"},
        {"id":"ac3","div":"academic","emoji":"📜","name":"Narratologist","vibe":"Every story is an argument — I help you find what yours is really saying","desc":"Expert in narrative theory, story structure, character arcs, and literary analysis — grounds advice in established frameworks from Propp to Campbell to modern narratology"},
        {"id":"ac4","div":"academic","emoji":"🗺️","name":"Geographer","vibe":"Geography is destiny — where you are determines who you become","desc":"Expert in physical and human geography, climate systems, cartography, and spatial analysis — builds geographically coherent worlds where terrain, climate, resources, and settlement patterns make scientific sense"},
        {"id":"ac5","div":"academic","emoji":"📚","name":"Historian","vibe":"History doesn't repeat, but it rhymes — and I know all the verses","desc":"Expert in historical analysis, periodization, material culture, and historiography — validates historical coherence and enriches settings with authentic period detail grounded in primary and secondary sources"},
        # ── DESIGN ────────────────────────────────────────────────
        {"id":"de1","div":"design","emoji":"📐","name":"UX Architect","vibe":"Gives developers solid foundations, CSS systems, and clear implementation paths","desc":"Technical architecture and UX specialist who provides developers with solid foundations, CSS systems, and clear implementation guidance"},
        {"id":"de2","div":"design","emoji":"✨","name":"Whimsy Injector","vibe":"Adds the unexpected moments of delight that make brands unforgettable","desc":"Expert creative specialist focused on adding personality, delight, and playful elements to brand experiences. Creates memorable, joyful interactions that differentiate brands through unexpected moments of whimsy"},
        {"id":"de3","div":"design","emoji":"🔬","name":"UX Researcher","vibe":"Validates design decisions with real user data, not assumptions","desc":"Expert user experience researcher specializing in user behavior analysis, usability testing, and data-driven design insights. Provides actionable research findings that improve product usability and user satisfaction"},
        {"id":"de4","div":"design","emoji":"🎨","name":"Brand Guardian","vibe":"Your brand's fiercest protector and most passionate advocate","desc":"Expert brand strategist and guardian specializing in brand identity development, consistency maintenance, and strategic brand positioning"},
        {"id":"de5","div":"design","emoji":"🎨","name":"UI Designer","vibe":"Creates beautiful, consistent, accessible interfaces that feel just right","desc":"Expert UI designer specializing in visual design systems, component libraries, and pixel-perfect interface creation. Creates beautiful, consistent, accessible user interfaces"},
        {"id":"de6","div":"design","emoji":"🎭","name":"Persona Walkthrough","vibe":"I become your user so you can see what your analytics can't show you","desc":"Simulate cognitive walkthroughs of web pages from a defined persona's psychological perspective — captures emotional reactions and rational thought at each scroll position, then delivers structured CRO reports"},
        {"id":"de7","div":"design","emoji":"📷","name":"Image Prompt Engineer","vibe":"Translates visual concepts into precise prompts that produce stunning AI photography","desc":"Expert photography prompt engineer specializing in crafting detailed, evocative prompts for AI image generation. Masters the art of translating visual concepts into precise language"},
        {"id":"de8","div":"design","emoji":"🌈","name":"Inclusive Visuals Specialist","vibe":"Defeats systemic AI biases to generate culturally accurate, affirming imagery","desc":"Representation expert who defeats systemic AI biases to generate culturally accurate, affirming, and non-stereotypical images and video"},
        {"id":"de9","div":"design","emoji":"🎬","name":"Visual Storyteller","vibe":"Transforms complex information into visual narratives that move people","desc":"Expert visual communication specialist focused on creating compelling visual narratives, multimedia content, and brand storytelling through design"},
        # ── ENGINEERING ───────────────────────────────────────────
        {"id":"en1","div":"engineering","emoji":"💎","name":"Senior Developer","vibe":"Premium full-stack craftsperson — Laravel, Livewire, Three.js, advanced CSS","desc":"Premium implementation specialist. Masters Laravel/Livewire/FluxUI, advanced CSS, Three.js integration. Creates premium web experiences with smooth animations and micro-interactions"},
        {"id":"en2","div":"engineering","emoji":"🏛️","name":"Software Architect","vibe":"Designs systems that survive the team that built them. Every decision has a trade-off — name it","desc":"Expert software architect specializing in system design, domain-driven design, architectural patterns, and technical decision-making for scalable, maintainable systems"},
        {"id":"en3","div":"engineering","emoji":"🏗️","name":"Backend Architect","vibe":"Designs the systems that hold everything up — databases, APIs, cloud, scale","desc":"Senior backend architect specializing in scalable system design, database architecture, API development, and cloud infrastructure. Builds robust, secure, performant server-side applications and microservices"},
        {"id":"en4","div":"engineering","emoji":"👁️","name":"Code Reviewer","vibe":"Reviews code like a mentor, not a gatekeeper. Every comment teaches something","desc":"Expert code reviewer who provides constructive, actionable feedback focused on correctness, maintainability, security, and performance — not style preferences"},
        {"id":"en5","div":"engineering","emoji":"📚","name":"Technical Writer","vibe":"Writes the docs that developers actually read and use","desc":"Expert technical writer specializing in developer documentation, API references, README files, and tutorials. Transforms complex engineering concepts into clear, accurate, and engaging docs"},
        {"id":"en6","div":"engineering","emoji":"⚡","name":"Rapid Prototyper","vibe":"Turns an idea into a working prototype before the meeting's over","desc":"Specialized in ultra-fast proof-of-concept development and MVP creation using efficient tools and frameworks"},
        {"id":"en7","div":"engineering","emoji":"📲","name":"Mobile App Builder","vibe":"Ships native-quality apps on iOS and Android, fast","desc":"Specialized mobile application developer with expertise in native iOS/Android development and cross-platform frameworks"},
        {"id":"en8","div":"engineering","emoji":"🪡","name":"Minimal Change Engineer","vibe":"The smallest diff that solves the problem — every extra line is a liability","desc":"Engineering specialist focused on minimum-viable diffs — fixes only what was asked, refuses scope creep, prefers three similar lines over a premature abstraction"},
        {"id":"en9","div":"engineering","emoji":"⚡","name":"Autonomous Optimization Architect","vibe":"The system governor that makes things faster without bankrupting you","desc":"Intelligent system governor that continuously shadow-tests APIs for performance while enforcing strict financial and security guardrails against runaway costs"},
        {"id":"en10","div":"engineering","emoji":"🔧","name":"Filament Optimization Specialist","vibe":"Pragmatic perfectionist — streamlines complex admin environments","desc":"Expert in restructuring and optimizing Filament PHP admin interfaces for maximum usability and efficiency. Focuses on impactful structural changes — not just cosmetic tweaks"},
        {"id":"en11","div":"engineering","emoji":"🔩","name":"Embedded Firmware Engineer","vibe":"Writes production-grade firmware for hardware that can't afford to crash","desc":"Specialist in bare-metal and RTOS firmware — ESP32/ESP-IDF, PlatformIO, Arduino, ARM Cortex-M, STM32, Nordic nRF5, FreeRTOS, Zephyr"},
        {"id":"en12","div":"engineering","emoji":"🛒","name":"Drupal Commerce Engineer","vibe":"A meticulous Drupal commerce engineer — the cart isn't a feature, it's a promise","desc":"Expert Drupal e-commerce engineer specializing in Drupal Commerce for product catalog management, payment gateway integration, checkout workflow design, and order management on Drupal 10/11"},
        {"id":"en13","div":"engineering","emoji":"🛍️","name":"WordPress Commerce Engineer","vibe":"Turns WooCommerce into powerful, conversion-optimized storefronts","desc":"Expert WordPress e-commerce engineer specializing in WooCommerce for product catalog management, payment gateway integration, checkout customization, and order management"},
        {"id":"en14","div":"engineering","emoji":"✉️","name":"Email Intelligence Engineer","vibe":"Extracts signal from email noise so agents can act on it","desc":"Expert in extracting structured, reasoning-ready data from raw email threads for AI agents and automation systems"},
        {"id":"en15","div":"engineering","emoji":"💬","name":"WeChat Mini Program Developer","vibe":"Builds performant Mini Programs that thrive in the WeChat ecosystem","desc":"Expert WeChat Mini Program developer specializing in 小程序 development with WXML/WXSS/WXS, WeChat API integration, payment systems, and subscription messaging"},
        {"id":"en16","div":"engineering","emoji":"🤖","name":"AI Engineer","vibe":"Ships AI features that work in production, not just demos","desc":"Expert AI engineer specializing in building production AI systems, RAG pipelines, LLM integrations, and AI-powered features that scale"},
        {"id":"en17","div":"engineering","emoji":"🔄","name":"DevOps Automator","vibe":"Automates the toil so engineers can ship faster","desc":"Expert DevOps engineer specializing in CI/CD pipelines, infrastructure as code, container orchestration, and deployment automation"},
        {"id":"en18","div":"engineering","emoji":"🌐","name":"Frontend Developer","vibe":"Builds fast, accessible, pixel-perfect frontends","desc":"Expert frontend developer specializing in modern JavaScript frameworks, performance optimization, accessibility, and responsive design"},
        {"id":"en19","div":"engineering","emoji":"🗃️","name":"Data Engineer","vibe":"Builds data pipelines that actually run in production","desc":"Expert data engineer specializing in ETL pipelines, data warehousing, streaming architectures, and data quality at scale"},
        {"id":"en20","div":"engineering","emoji":"🔍","name":"Database Optimizer","vibe":"Finds the slow query, kills it, and makes sure it never comes back","desc":"Expert database performance engineer specializing in query optimization, indexing strategies, and database architecture for high-traffic applications"},
        # ── FINANCE ───────────────────────────────────────────────
        {"id":"fi1","div":"finance","emoji":"📊","name":"Financial Analyst","vibe":"Turns numbers into decisions","desc":"Expert financial analyst specializing in financial modeling, valuation, variance analysis, and investment research. Provides data-driven financial insights"},
        {"id":"fi2","div":"finance","emoji":"📒","name":"Bookkeeper Controller","vibe":"Every cent has a story — I make sure it's the right one","desc":"Expert bookkeeper and controller specializing in general ledger management, month-end close, reconciliation, and financial reporting accuracy"},
        {"id":"fi3","div":"finance","emoji":"📈","name":"FP&A Analyst","vibe":"Bridges strategy and the spreadsheet","desc":"Expert Financial Planning & Analysis specialist focused on budgeting, forecasting, scenario modeling, and management reporting"},
        {"id":"fi4","div":"finance","emoji":"🔎","name":"Investment Researcher","vibe":"Finds the signal before the market does","desc":"Expert investment researcher specializing in equity analysis, sector research, competitive intelligence, and investment thesis development"},
        {"id":"fi5","div":"finance","emoji":"🏛️","name":"Tax Strategist","vibe":"Legally keeps more of what you earn","desc":"Expert tax strategist specializing in tax planning, compliance, optimization strategies, and navigating complex tax regulations for individuals and businesses"},
        # ── GAME DEVELOPMENT ──────────────────────────────────────
        {"id":"gm1","div":"game-development","emoji":"🎮","name":"Game Designer","vibe":"Every mechanic should create a story worth telling","desc":"Expert game designer specializing in game mechanics, systems design, player psychology, and creating engaging gameplay loops across all genres and platforms"},
        {"id":"gm2","div":"game-development","emoji":"🎵","name":"Game Audio Engineer","vibe":"Sound is 50% of the experience — treat it that way","desc":"Expert game audio engineer specializing in dynamic audio systems, spatial audio, adaptive music, and sound design for immersive gaming experiences"},
        {"id":"gm3","div":"game-development","emoji":"🗺️","name":"Level Designer","vibe":"Every space tells the player what to do next without saying a word","desc":"Expert level designer specializing in spatial design, player flow, environmental storytelling, and pacing for compelling level experiences"},
        {"id":"gm4","div":"game-development","emoji":"📖","name":"Narrative Designer","vibe":"Story and systems should be inseparable","desc":"Expert narrative designer specializing in interactive storytelling, branching dialogue, world-building, and integrating narrative with gameplay mechanics"},
        {"id":"gm5","div":"game-development","emoji":"🎨","name":"Technical Artist","vibe":"The bridge between art and code — makes both sides look good","desc":"Expert technical artist specializing in shader development, VFX, pipeline tools, and optimizing art assets for real-time rendering in game engines"},
        # ── MARKETING ─────────────────────────────────────────────
        {"id":"mk1","div":"marketing","emoji":"🔍","name":"SEO Specialist","vibe":"Ranks pages that deserve to rank, for queries that convert","desc":"Expert SEO specialist with deep knowledge of technical SEO, content strategy, link building, and search algorithm optimization for sustainable organic growth"},
        {"id":"mk2","div":"marketing","emoji":"📧","name":"Email Strategist","vibe":"The inbox is a relationship — every send is a trust transaction","desc":"Expert email marketing strategist specializing in list segmentation, automation sequences, deliverability, A/B testing, and lifecycle email programs"},
        {"id":"mk3","div":"marketing","emoji":"📱","name":"TikTok Strategist","vibe":"Viral is a system, not luck — here's the system","desc":"Expert TikTok strategist specializing in content hooks, trend leveraging, algorithm optimization, and building creator-led growth on TikTok"},
        {"id":"mk4","div":"marketing","emoji":"📣","name":"Social Media Strategist","vibe":"Builds communities, not just audiences","desc":"Expert social media strategist specializing in multi-platform content strategy, community management, influencer partnerships, and social analytics"},
        {"id":"mk5","div":"marketing","emoji":"✍️","name":"Content Creator","vibe":"Content that educates earns trust. Trust earns sales","desc":"Expert content creator specializing in SEO-optimized blogs, thought leadership, video scripts, and content strategies that drive measurable business outcomes"},
        {"id":"mk6","div":"marketing","emoji":"💼","name":"LinkedIn Content Creator","vibe":"Turns expertise into authority on LinkedIn","desc":"Expert LinkedIn content strategist specializing in thought leadership posts, personal brand building, B2B engagement, and LinkedIn algorithm optimization"},
        {"id":"mk7","div":"marketing","emoji":"🌱","name":"Growth Hacker","vibe":"Finds the leverage point where small effort creates big growth","desc":"Expert growth hacker specializing in rapid experimentation, product-led growth, acquisition funnels, retention optimization, and viral loop design"},
        {"id":"mk8","div":"marketing","emoji":"🎙️","name":"Podcast Strategist","vibe":"Every great podcast starts with a promise to the audience — what's yours?","desc":"Expert podcast strategist specializing in show concept development, audience growth, monetization strategy, and multi-platform distribution"},
        # ── PAID MEDIA ────────────────────────────────────────────
        {"id":"pm1","div":"paid-media","emoji":"💰","name":"PPC Strategist","vibe":"Every dollar should have a job — and a performance review","desc":"Expert PPC strategist specializing in Google Ads, Bing Ads, search campaign architecture, bid strategies, and conversion rate optimization for paid search"},
        {"id":"pm2","div":"paid-media","emoji":"📊","name":"Paid Social Strategist","vibe":"Audience targeting is the strategy — creative is the execution","desc":"Expert paid social specialist for Meta, TikTok, LinkedIn, and Pinterest advertising — builds full-funnel campaigns with strong creative testing frameworks"},
        {"id":"pm3","div":"paid-media","emoji":"🎯","name":"Paid Media Auditor","vibe":"Finds the budget leaks before they sink the account","desc":"Expert paid media auditor who systematically reviews ad accounts for waste, missed opportunities, tracking gaps, and structural inefficiencies"},
        {"id":"pm4","div":"paid-media","emoji":"🖥️","name":"Programmatic Buyer","vibe":"Precision targeting at scale — the right ad, right person, right moment","desc":"Expert programmatic advertising specialist for DSP platforms, audience targeting, brand safety, viewability, and data-driven display and video buying"},
        # ── PRODUCT ───────────────────────────────────────────────
        {"id":"pr1","div":"product","emoji":"📦","name":"Product Manager","vibe":"Ships the right thing, not just the next thing","desc":"Expert product manager specializing in roadmap prioritization, stakeholder alignment, user research, and building products that solve real customer problems"},
        {"id":"pr2","div":"product","emoji":"🧩","name":"Sprint Prioritizer","vibe":"Not everything is urgent — but some things actually are","desc":"Expert sprint planning specialist who helps teams cut scope intelligently, sequence work by value and risk, and ship increments that matter"},
        {"id":"pr3","div":"product","emoji":"💡","name":"Feedback Synthesizer","vibe":"Turns a thousand opinions into three decisions","desc":"Expert at synthesizing user feedback, support tickets, NPS data, and qualitative research into actionable product insights and prioritized feature themes"},
        {"id":"pr4","div":"product","emoji":"🔮","name":"Trend Researcher","vibe":"Spots the signal in the market noise before it becomes obvious","desc":"Expert market and trend researcher who identifies emerging opportunities, competitive shifts, and technology trends that should inform product strategy"},
        # ── PROJECT MANAGEMENT ────────────────────────────────────
        {"id":"pj1","div":"project-management","emoji":"📋","name":"Project Shepherd","vibe":"Projects don't fail at the end — they fail in week two","desc":"Expert project manager who keeps complex, multi-stakeholder projects on track through clear milestones, risk management, and proactive communication"},
        {"id":"pj2","div":"project-management","emoji":"🗂️","name":"Jira Workflow Steward","vibe":"Jira should clarify work, not create more of it","desc":"Expert Jira administrator and workflow designer who builds board structures, automation rules, and reporting that actually help teams ship faster"},
        {"id":"pj3","div":"project-management","emoji":"📝","name":"Meeting Notes Specialist","vibe":"Good notes are the memory of the team","desc":"Expert at capturing, structuring, and distributing meeting notes with clear action items, owners, and deadlines so decisions don't get lost"},
        {"id":"pj4","div":"project-management","emoji":"🎬","name":"Studio Producer","vibe":"Keeps creative teams delivering without burning out","desc":"Expert studio producer specializing in creative project management, resource allocation, deadline management, and cross-functional coordination in creative studios"},
        # ── SALES ─────────────────────────────────────────────────
        {"id":"sa1","div":"sales","emoji":"🏆","name":"Sales Coach","vibe":"Selling is serving — help them buy what solves their problem","desc":"Expert sales coach specializing in discovery methodology, objection handling, pipeline development, and building high-performing sales teams through structured coaching"},
        {"id":"sa2","div":"sales","emoji":"🤝","name":"Account Strategist","vibe":"Accounts grow when you know them better than they know themselves","desc":"Expert account strategist specializing in enterprise account planning, expansion strategies, executive relationship management, and multi-threaded deal execution"},
        {"id":"sa3","div":"sales","emoji":"📨","name":"Outbound Strategist","vibe":"Cold outreach works — bad outreach doesn't","desc":"Expert outbound sales strategist specializing in prospecting sequences, personalization at scale, ICP targeting, and converting cold leads to qualified pipeline"},
        {"id":"sa4","div":"sales","emoji":"🔬","name":"Discovery Coach","vibe":"The best salespeople ask questions — then listen","desc":"Expert at teaching structured discovery frameworks that uncover true buyer motivations, hidden objections, and decision criteria before proposing solutions"},
        {"id":"sa5","div":"sales","emoji":"📄","name":"Proposal Strategist","vibe":"A proposal is a closing document, not a brochure","desc":"Expert proposal writer and strategist who builds compelling, tailored proposals that address buyer concerns, quantify value, and accelerate close rates"},
        # ── SECURITY ──────────────────────────────────────────────
        {"id":"se1","div":"security","emoji":"🛡️","name":"Security Architect","vibe":"Security is a property of the system, not a feature you add later","desc":"Expert security architect specializing in zero-trust architecture, threat modeling, security design reviews, and building security-by-design into complex systems"},
        {"id":"se2","div":"security","emoji":"🔐","name":"AppSec Engineer","vibe":"Finds the vulnerability before the attacker does — then fixes it properly","desc":"Expert application security engineer specializing in SAST/DAST, secure code review, OWASP Top 10 remediation, and building secure SDLC practices"},
        {"id":"se3","div":"security","emoji":"🚨","name":"Incident Responder","vibe":"In an incident, calm is the most important skill","desc":"Expert incident response specialist with deep experience in triage, containment, forensic investigation, root cause analysis, and post-incident remediation"},
        {"id":"se4","div":"security","emoji":"☁️","name":"Cloud Security Architect","vibe":"The shared responsibility model only works if you take your half seriously","desc":"Expert cloud security architect specializing in AWS/Azure/GCP security posture, IAM design, data protection, and cloud-native security controls"},
        {"id":"se5","div":"security","emoji":"🕵️","name":"Penetration Tester","vibe":"Finds the holes so the attackers don't find them first","desc":"Expert penetration tester specializing in network, web application, and social engineering assessments. Delivers clear, actionable findings with business impact context"},
        {"id":"se6","div":"security","emoji":"📋","name":"Compliance Auditor","vibe":"Compliance is the floor — security is the goal","desc":"Expert compliance specialist for SOC 2, ISO 27001, HIPAA, GDPR, and PCI DSS — builds audit-ready programs that satisfy regulators without strangling engineering"},
        {"id":"se7","div":"security","emoji":"🔍","name":"Threat Intelligence Analyst","vibe":"Understands the adversary so you can stay one step ahead","desc":"Expert threat intelligence analyst who tracks threat actors, analyzes TTPs, and delivers actionable intelligence to help organizations proactively defend against targeted attacks"},
        # ── SPECIALIZED ───────────────────────────────────────────
        {"id":"sp1","div":"specialized","emoji":"🤖","name":"Agents Orchestrator","vibe":"Coordinates AI agents so they amplify each other instead of colliding","desc":"Expert in multi-agent system design, agent orchestration patterns, task delegation, and building reliable agentic workflows that complete complex goals autonomously"},
        {"id":"sp2","div":"specialized","emoji":"🏢","name":"Business Strategist","vibe":"Strategy is the choices you make — and the ones you refuse to make","desc":"Expert business strategist specializing in competitive analysis, market positioning, growth strategy, and translating strategic vision into executable roadmaps"},
        {"id":"sp3","div":"specialized","emoji":"🧑‍💼","name":"Chief of Staff","vibe":"Makes leadership 10x more effective by absorbing the complexity around them","desc":"Expert chief of staff specializing in executive coordination, strategic projects, cross-functional alignment, and turning leadership intent into team execution"},
        {"id":"sp4","div":"specialized","emoji":"🔧","name":"MCP Builder","vibe":"Builds Model Context Protocol servers that extend what AI can do","desc":"Expert in building MCP servers, tool definitions, and AI capability extensions that integrate AI assistants with external systems and data sources"},
        {"id":"sp5","div":"specialized","emoji":"⚖️","name":"Data Privacy Officer","vibe":"Privacy isn't a checkbox — it's a design principle","desc":"Expert data privacy officer specializing in GDPR, CCPA, privacy impact assessments, consent management, and building privacy-by-design into products"},
        {"id":"sp6","div":"specialized","emoji":"🌍","name":"Cultural Intelligence Strategist","vibe":"What works in one culture can fail catastrophically in another","desc":"Expert cultural intelligence strategist who helps organizations navigate cross-cultural communication, global expansion, and culturally-adaptive product and marketing strategies"},
        # ── SUPPORT ───────────────────────────────────────────────
        {"id":"su1","div":"support","emoji":"🛟","name":"Support Responder","vibe":"A great support response turns a frustrated customer into a loyal one","desc":"Expert customer support specialist who crafts empathetic, clear, and effective responses that resolve issues on first contact and build lasting customer relationships"},
        {"id":"su2","div":"support","emoji":"📊","name":"Analytics Reporter","vibe":"Support data tells you what to fix before customers stop asking","desc":"Expert at analyzing support metrics, ticket trends, CSAT data, and escalation patterns to surface product and process improvement opportunities"},
        {"id":"su3","div":"support","emoji":"⚙️","name":"Infrastructure Maintainer","vibe":"Reliability is a feature — the most important one","desc":"Expert infrastructure engineer specializing in monitoring, incident management, capacity planning, and keeping production systems reliable and performant"},
        {"id":"su4","div":"support","emoji":"📋","name":"Legal Compliance Checker","vibe":"Finds the legal risk before it finds the company","desc":"Expert at reviewing business processes, contracts, and communications for legal compliance risks, helping teams stay within regulatory and contractual boundaries"},
        # ── TESTING ───────────────────────────────────────────────
        {"id":"te1","div":"testing","emoji":"🧪","name":"API Tester","vibe":"If the contract breaks, everything breaks — test the contract","desc":"Expert API testing specialist for REST, GraphQL, and gRPC — builds comprehensive test suites covering functional correctness, edge cases, and integration reliability"},
        {"id":"te2","div":"testing","emoji":"♿","name":"Accessibility Auditor","vibe":"Accessibility is quality — if it excludes anyone, it's not finished","desc":"Expert accessibility auditor specializing in WCAG 2.1/2.2, screen reader testing, keyboard navigation, and color contrast — delivers actionable remediation guidance"},
        {"id":"te3","div":"testing","emoji":"⚡","name":"Performance Benchmarker","vibe":"Slow is a bug — measure it, fix it, prove it's fixed","desc":"Expert performance testing specialist for load testing, stress testing, profiling, and identifying bottlenecks across web applications and APIs"},
        {"id":"te4","div":"testing","emoji":"✅","name":"Reality Checker","vibe":"Assumption is the enemy of quality — verify everything","desc":"Expert at validating requirements, testing edge cases, questioning assumptions, and ensuring software actually does what it's supposed to do in the real world"},
        {"id":"te5","div":"testing","emoji":"🔧","name":"Workflow Optimizer","vibe":"The best process is the one the team actually follows","desc":"Expert at analyzing, documenting, and improving development and QA workflows — reduces friction, eliminates bottlenecks, and builds sustainable engineering practices"},
    ]
    return render_template(
        "agents.html",
        divisions=divisions,
        tools=tools,
        divisions_json=_json.dumps(divisions),
...
def api_manuals_upload():
    ...
    model = (
        input_validator.validate_string(request.form.get("model"), max_length=80) or ""
    )
    year = (
        input_validator.validate_string(request.form.get("year"), max_length=16) or ""
    )
    default_title = " ".join(
        part for part in [brand or "", model, year] if part
    ).strip()
    title = (
        input_validator.validate_string(request.form.get("title"), max_length=200)
        or default_title
    )
    description = (
        input_validator.validate_string(
    ...
    license_name = (
        input_validator.validate_string(request.form.get("license"), max_length=200)
        or ""
    )
    source_url_value = (request.form.get("source_url") or "").strip()
    source_url = (
        input_validator.validate_url(source_url_value) if source_url_value else ""
    )
    ...
    new_id = manual_store.add_manual(
        brand=brand,
        model=model,
        year=year,
        title=title,
...
def api_manuals_import_wikimedia():
    ...
    query = input_validator.validate_string(
        payload.get("query") or f"{brand} {model}",
        max_length=240,
    )
                ...
                if "public domain" in lic or "cc0" in lic or lic == "pd":
                    try:
                        ...
                        with open(save_path, "wb") as fh:
                            fh.writelines(r.iter_content(8192))
                        downloaded.append(
                            f"/static/manuals/images/{mid}/{os.path.basename(save_path)}",
                        )
                    except Exception:
                        continue
            # Create a stub manual entry to hold these images
            title = f"{brand} {model} {year} (Wikimedia images)"
            mid_saved = manual_store.add_manual(
                brand=brand,
                model=model,
                year=year,
                title=title,
                description="Imported from Wikimedia Commons",
                license="various",
                source_url=None,
                pdf_path=None,
